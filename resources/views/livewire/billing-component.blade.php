<div class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-[#e2e4ea] p-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h2 class="text-[13.5px] font-semibold text-[#353636] mb-1">{{ __('portal.your_subscription') }}</h2>
                    ...
                    <div class="flex items-center gap-3 mt-2">
                        ...
                        @if ($presenter->renewsAt() !== '—')
                            <span class="text-[12px] text-[#7c7c86]">
                                {{ __('portal.renews_on', ['date' => $presenter->renewsAt()]) }}
                            </span>
                        @endif
                    </div>
                @else
                    <p class="text-[20px] font-semibold text-[#7c7c86]">{{ __('portal.no_active_plan') }}</p>
                    <p class="text-sm text-[#9ca3af] mt-1">
                        {{ __('portal.choose_plan_desc') }}
                    </p>
                @endif
            </div>
            ...
            <div class="mt-5 pt-5 border-t border-[#f3f4f6] flex items-center gap-3">
                ...
                <div>
                    <p class="text-[13px] font-medium text-[#353636] capitalize">
                        {{ __('portal.pm_label', ['type' => $presenter->pmType(), 'last4' => $presenter->pmLastFour()]) }}
                    </p>
                    <p class="text-[11.5px] text-[#7c7c86]">{{ __('portal.active_pm') }}</p>
                </div>
            </div>
        @endif
    </div>
    ...
    <div>
        <h2 class="text-[13.5px] font-semibold text-[#353636] mb-4">
            {{ $presenter->hasSubscription() ? __('portal.change_plan') : __('portal.choose_plan') }}
        </h2>
                ...
                <div class="relative bg-white rounded-2xl border p-6 flex flex-col gap-4
                    ...
                    @else
                        <button
                            ...
                            <span wire:loading.remove wire:target="checkout">{{ __('portal.choose_name', ['name' => $plan['name']]) }}</span>
                            <span wire:loading wire:target="checkout">{{ __('portal.redirecting') }}</span>
                        </button>
                    @endif
                </div>
            @endforeach
